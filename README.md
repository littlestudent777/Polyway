...
**YOLOv4** - state-of-the-art алгоритм для детекции объектов в реальном времени, обеспечивающий оптимальный баланс между скоростью и точностью.

#### Ключевые особенности:
- Молниеносная обработка изображений (real-time performance)
- Высокая точность распознавания объектов
- Поддержка детекции множества классов объектов

### TensorFlow Lite
Оптимизированный фреймворк для выполнения ML-моделей на мобильных устройствах.

#### Преимущества в проекте:
- Поддержка Java/Kotlin API для Android
- Эффективное использование ресурсов устройства
- Поддержка аппаратного ускорения (GPU/NNAPI)

### Dataset
В качестве датасета для обучения и тренировки нейросети был выбран AffectNet (Subset)
